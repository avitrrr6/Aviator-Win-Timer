...
exports.getWalletByUserId = async (req, res) => {
  try {
    ...
    const data = await queryDb(query_for_get_balance, [Number(user_id)]);
...
exports.getTopRecordsAviator = async (req, res) => {
  try {
    const query_for_find_top_winner =
      "SELECT a.`amount`,a.`amountcashed`,a.`multiplier`,a.`createdAt`,a.`updatedAt`,u.`or_m_email`,u.`or_m_name`,u.`or_m_dob`,u.or_m_user_id FROM `aviator_bet_place_ledger` AS a LEFT JOIN `m03_user_detail` AS u ON a.`userid` = u.`or_m_reg_id` ORDER BY a.`amountcashed` DESC LIMIT 10;";
